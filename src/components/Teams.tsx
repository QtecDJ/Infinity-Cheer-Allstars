...
const TeamCard = ({ name, category, ageGroup, description, focus, icon, colorAccent }: TeamCardProps) => {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      whileHover={{ y: -10 }}
      className="bg-white border-4 md:border-8 border-black shadow-2xl overflow-hidden group"
    >
      {/* Team Color Stripe */}
      <div className={`h-4 md:h-6 ${colorAccent}`}></div>
      
      <div className="p-6 md:p-10">
        {/* Icon */}
        <div className="w-16 h-16 md:w-24 md:h-24 bg-black border-4 border-brand-red mx-auto mb-4 md:mb-6 flex items-center justify-center group-hover:scale-110 transition-transform">
          {icon}
        </div>
        
        {/* Team Name */}
        <h3 className="text-3xl md:text-4xl lg:text-5xl font-black text-black mb-2 uppercase tracking-tight text-center" style={{
          textShadow: '2px 2px 0 #DC2626'
        }}>{name}</h3>
        
        {/* Category Badge */}
        <div className="inline-block bg-black border-4 border-brand-red px-4 py-1 md:px-6 md:py-2 mx-auto mb-4 md:mb-6">
          <span className="text-base md:text-xl font-black text-brand-red uppercase tracking-wider">{category}</span>
        </div>
  ...
  return (
    <section id="teams" className="relative py-32 bg-gray-100">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        >
          <div className="inline-block bg-brand-red border-4 md:border-8 border-black px-6 py-3 md:px-10 md:py-4 mb-6 md:mb-8">
            <h2 className="text-2xl md:text-4xl font-black text-white uppercase tracking-widest">Our Teams</h2>
          </div>
